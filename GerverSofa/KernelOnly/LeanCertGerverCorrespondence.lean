...
@[simp] theorem eval_scaledVar {n : Nat} (box : List RatInterval)
    (u : Fin n → ℝ) (i : Fin n) :
    evalFin (scaledVar box i.1) u = affineFromBox box u i := by
  simp [evalFin, finEnv, scaledVar, affineFromBox, eadd, emul, ec, ev]

@[simp] theorem eval_epi {n : Nat} (u : Fin n → ℝ) :
    evalFin epi u = Real.pi := by
  simp [evalFin, epi, finEnv]
...
theorem fullNormalize_mem {x : Fin 22 → ℝ}
    (hx : x ∈ Romik.vectorBox) :
    FinBoxMem (fullNormalize x) (unitBox (n := 22)) := by
  have henc := (Romik.inputBox_exact x).1 hx
  intro i
  have hi := henc.2 i
  have h := normalize_mem_unit
    (ExactReplay.getI ExactReplay.fullInputBox i.1)
    (full_box_strict i) (x i) hi
  simpa [fullNormalize, normalizeToBox, boxMid, boxRad, unitBox,
    IntervalRat.mem_def] using h
...
theorem fullAffine_normalize (x : Fin 22 → ℝ) :
    fullAffine (fullNormalize x) = x := by
  funext i
  simpa [fullAffine, fullNormalize, affineFromBox, normalizeToBox,
    boxMid, boxRad] using
      affine_normalize_coord
        (ExactReplay.getI ExactReplay.fullInputBox i.1)
        (full_box_strict i) (x i)
