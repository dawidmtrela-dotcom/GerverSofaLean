...
/-- First projection of `fullDualOutput` is byte-level the executable AD call. -/
theorem fullDualOutput_d_eq :
    fullDualOutput.map SoundDual.d =
      ExactReplay.fullIntervalSystem ExactReplay.fullInputBox := by
  set_option maxRecDepth 100000 in
    rfl

/-- The proof-carrying output has exactly 22 scalar components. -/
theorem fullDualOutput_length : fullDualOutput.length = 22 := by
  norm_num [fullDualOutput]
...
/-- Every exact direct-system output dual is sound for the corresponding model. -/
theorem fullDualOutput_sound (i : Fin 22) :
    DSoundOn vectorBox
      ((ExactReplay.fullIntervalSystem ExactReplay.fullInputBox).getD i.1
        (ExactReplay.D.const ExactReplay.zeroI 22))
      ((fullDualOutput.getD i.1 (SoundDual.pointConst 0)).model) := by
  have hi : i.1 < fullDualOutput.length := by
    simpa [fullDualOutput_length] using i.2
  have hproj := congrArg (fun xs => xs.getD i.1
      (ExactReplay.D.const ExactReplay.zeroI 22)) fullDualOutput_d_eq
  have hget :
      (fullDualOutput.map SoundDual.d).getD i.1
          (ExactReplay.D.const ExactReplay.zeroI 22) =
        (fullDualOutput.getD i.1 (SoundDual.pointConst 0)).d := by
    simp [hi, SoundDual.pointConst, ExactReplay.D.pointConst]
  rw [hget] at hproj
  rw [← hproj]
  exact (fullDualOutput.getD i.1 (SoundDual.pointConst 0)).sound

/-- Concrete semantic correctness of the complete direct 22D interval-AD call. -/
def fullADSoundness : ADSystemSoundness
    vectorSystem vectorBox ExactReplay.fullInputBox
      ExactReplay.fullIntervalSystem where
  input_exact := inputBox_exact
  output_sound := by
    intro x hx
    apply adOutputSoundAt_of_models
      (X := vectorBox)
      (out := ExactReplay.fullIntervalSystem ExactReplay.fullInputBox)
      (models := fun i =>
        (fullDualOutput.getD i.1 (SoundDual.pointConst 0)).model)
      (F := vectorSystem)
    · have hlenMap : (fullDualOutput.map SoundDual.d).length = 22 := by
        simpa using fullDualOutput_length
      rw [fullDualOutput_d_eq] at hlenMap
      exact hlenMap
    · exact fullDualOutput_sound
    · intro y
      symm
      exact fullDualOutput_model_eq y
    · exact hx
