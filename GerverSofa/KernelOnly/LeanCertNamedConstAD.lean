...
/-- There are no partial-domain operators in `ADConstSupported`. -/
theorem ADConstSupported.domainValid {e : Expr} (h : ADConstSupported e)
...
theorem newtonMap_center_mem_const {n : Nat}
  ...
  · simp only [Matrix.mulVec, dotProduct, intervalRatMatVec]
    exact mem_interval_sum Finset.univ
      ...
      (fun j => IntervalRat.scale (Y i j)
        (pointEvalIntervals F m cfg j))
      (fun j _ => IntervalRat.mem_scale (Y i j)
        (systemEval_mem_pointEvalIntervals_const F h m cfg j))
