...
/-- Concrete analytic proof object for the exact transcendental evaluator. -/
def exactTranscendentalSoundness : TranscendentalSoundness where
  pi_mem := ExactReplay.piI_contains_pi
  sine_mem := by
    intro z x hz hx0 hxpi
    exact ExactReplay.sinI_contains hz hx0 hxpi
  cosine_mem := by
    intro z x hz hx0 hxpi
    exact ExactReplay.cosI_contains hz hx0 hxpi
