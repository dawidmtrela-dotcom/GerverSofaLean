import GerverSofa.KernelOnly.PartE.RegionalCoverDecomposition
...
def FiniteRejectionCoverOutsideLocalTarget (cells : List AngleCell)
    (cfg : EvalConfig := {}) : Prop :=
...
theorem scaledResidualEnclosureTarget_of_wideLocalCover
    (cells : List AngleCell) (cfg : EvalConfig)
    (hcover : FiniteRejectionCoverOutsideLocalTarget cells cfg)
    (hlocal : WideLocalReconstructionEnclosureTarget) :
    ScaledResidualEnclosureTarget := by
  intro phi theta hspec
  by_cases hin : localAngleCell.Contains phi theta
  · exact hlocal phi theta hspec hin
  · have hcovered : coveredBy cells phi theta :=
      hcover.2 phi theta hspec.1 hin
...
/-- Exact DeepMind-shaped terminal theorem.  After E21 the only remaining
data object is a finite rejected cover outside `localAngleCell`; no refinement
against the microscopic boundary of `Reduced.box` is required. -/
theorem deepMindABPhiTheta_existsUnique_of_wideLocalCover
    (cells : List AngleCell) (cfg : EvalConfig)
    (hcover : FiniteRejectionCoverOutsideLocalTarget cells cfg) :
    ∃! ABphiTheta : ℝ × ℝ × ℝ × ℝ,
  ...
  deepMindABPhiTheta_existsUnique_of_twoAngleEnclosure
    ((scaledResidualEnclosureTarget_iff_twoAngleEnclosureTarget).1
      (scaledResidualEnclosureTarget_of_wideLocalCover
        cells cfg hcover wideLocalReconstructionEnclosureTarget))
