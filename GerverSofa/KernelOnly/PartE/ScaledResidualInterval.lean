...
/-- Certified interval evaluation of one smooth residual. -/
def scaledResidualInterval (i : Fin 2) (phiI thetaI : IntervalRat)
    (cfg : EvalConfig := {}) : IntervalRat :=
  LeanCert.Internal.Rational.evalTotalCore
    (scaledResidualExpr i) (angleIntervalEnv phiI thetaI) cfg

/-- LeanCert interval evaluation encloses the corresponding real residual. -/
theorem scaledResidual_mem_interval (i : Fin 2)
    ...
    (if i = 0 then scaledResidualOne phi theta
      ...
      scaledResidualInterval i phiI thetaI cfg := by
  ...
  have henv : envMem (finEnv u) (angleIntervalEnv phiI thetaI) := by
    ...
    · change finEnv u (Nat.succ (Nat.succ n)) ∈ (default : IntervalRat)
      ...
      simp [finEnv]
  have hcore := evalIntervalCore_correct
    (scaledResidualExpr i) (scaledResidualExpr_supported i).toCore
    (finEnv u) (angleIntervalEnv phiI thetaI) henv cfg
    ((scaledResidualExpr_supported i).domainValid
      (angleIntervalEnv phiI thetaI) cfg)
  change evalFin (scaledResidualExpr i) u ∈
    scaledResidualInterval i phiI thetaI cfg at hcore
  fin_cases i
...
/-- Kernel theorem for one branch-and-bound rectangle.  If either certified
residual interval excludes zero, the rectangle contains no common zero. -/
theorem no_common_zero_of_interval_rejection
    (phi theta : ℝ) (phiI thetaI : IntervalRat)
    (hphi : phi ∈ phiI) (htheta : theta ∈ thetaI)
    (cfg : EvalConfig := {})
    (hreject :
      intervalExcludesZero
          (scaledResidualInterval (0 : Fin 2) phiI thetaI cfg) = true ∨
      intervalExcludesZero
          (scaledResidualInterval (1 : Fin 2) phiI thetaI cfg) = true) :
    ¬ (scaledResidualOne phi theta = 0 ∧
      scaledResidualTwo phi theta = 0) := by
  rintro ⟨hzero1, hzero2⟩
  rcases hreject with hreject | hreject
  · have hmem := scaledResidual_mem_interval (0 : Fin 2)
      phi theta phiI thetaI hphi htheta cfg
    have hnot := intervalExcludesZero_sound _ hreject
    apply hnot
    simpa [hzero1] using hmem
  · have hmem := scaledResidual_mem_interval (1 : Fin 2)
      phi theta phiI thetaI hphi htheta cfg
    have hnot := intervalExcludesZero_sound _ hreject
    apply hnot
