...
/-! ## From a list of scalar models to `ADOutputSoundAt` -/

/-- Assemble sound scalar outputs into the matrix-oriented interface used by
`ADSystemSoundness`. -/
theorem adOutputSoundAt_of_models {n : Nat} {X : Set (Vec n)}
    (out : List ExactReplay.D) (models : Fin n → ScalarModel n)
    (houtLength : out.length = n)
    (hout : ∀ i : Fin n,
      DSoundOn X (out.getD i.1 (ExactReplay.D.const ExactReplay.zeroI n))
        (models i))
    (F : Vec n → Vec n)
    (hF : ∀ x, F x = fun i => (models i).value x) :
    ∀ x ∈ X, ADOutputSoundAt F x out := by
  intro x hx
  let J : Matrix (Fin n) (Fin n) ℝ := fun i j => (models i).gradient x j
  refine ⟨J, ?_, ?_, ?_⟩
  · intro i j
    simpa [J, hF] using (models i).hasDeriv_update x j
  · constructor
    · simpa [valueIntervals] using houtLength
    · intro i
      have hi : i.1 < out.length := by simpa [houtLength] using i.2
      rw [show (valueIntervals out).getD i.1 (RatInterval.point 0) =
          (out.getD i.1 (ExactReplay.D.const ExactReplay.zeroI n)).val by
        simp [valueIntervals, getD_map_of_lt, hi, ExactReplay.D.const,
          ExactReplay.zeroI]]
      simpa [hF] using (hout i).value_sound x hx
  · constructor
    · simpa [derivativeRows] using houtLength
    · intro i j
      have hi : i.1 < out.length := by simpa [houtLength] using i.2
      rw [show ((derivativeRows out).getD i.1 []).getD j.1 (RatInterval.point 0) =
          (out.getD i.1 (ExactReplay.D.const ExactReplay.zeroI n)).der.getD
            j.1 ExactReplay.zeroI by
        simp [derivativeRows, getD_map_of_lt, hi, ExactReplay.D.const,
          ExactReplay.zeroI]]
      exact (hout i).derivative_sound x hx j
