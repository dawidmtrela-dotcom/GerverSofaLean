...
/-- Concrete direct 22D unique zero — no assumptions. -/
def fullCertifiedUniqueZero :
    CertifiedUniqueZero Romik.vectorSystem Romik.vectorBox where
  solution := fullRoot
  solution_mem := fullRoot_mem
  satisfies := fullRoot_zero
  unique := by
    intro y hy hzero
    let uy := fullNormalize y
    have huy : FinBoxMem uy (unitBox (n := 22)) := fullNormalize_mem hy
    have hsys : SystemZero fullExpr uy := by
      intro i
      change systemEval fullExpr uy i = 0
      calc
        systemEval fullExpr uy i =
            Romik.vectorSystem (fullAffine uy) i :=
          congrFun (full_systemEval_eq uy) i
        _ = Romik.vectorSystem y i := by
          rw [show fullAffine uy = y by
            simpa [uy] using fullAffine_normalize y]
        _ = 0 := congrFun hzero i
    have huEq : uy = fullRootU := by
      exact full_unique_scaled.unique ⟨huy, hsys⟩ fullRootU_spec
    calc
      y = fullAffine uy := by symm; exact fullAffine_normalize y
      _ = fullAffine fullRootU := by rw [huEq]
      _ = fullRoot := rfl
...
def fullCertifiedUniqueSolution :
    CertifiedUniqueSolution Romik.Equations Romik.box :=
  Romik.uniqueSolutionOfVector fullCertifiedUniqueZero
