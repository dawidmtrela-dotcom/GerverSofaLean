...
def validB (x : RatInterval) : Bool := decide (x.lo ≤ x.hi)

/-- Closed interval inclusion. -/
def subsetB (x outer : RatInterval) : Bool :=
...
@[simp] theorem validB_eq_true_iff (z : RatInterval) :
    validB z = true ↔ z.lo ≤ z.hi := by
  simp [validB]

@[simp] theorem subsetB_eq_true_iff (x outer : RatInterval) :
    subsetB x outer = true ↔ outer.lo ≤ x.lo ∧ x.hi ≤ outer.hi := by
  simp [subsetB]
...
/-- Length-sensitive pointwise strict inclusion. -/
def allStrictInsideB : List RatInterval → List RatInterval → Bool
  | [], [] => true
  | x :: xs, y :: ys => strictInsideB x y && allStrictInsideB xs ys
  | _, _ => false

/-- Length-sensitive pointwise closed inclusion. -/
def allSubsetB : List RatInterval → List RatInterval → Bool
  | [], [] => true
  | x :: xs, y :: ys => subsetB x y && allSubsetB xs ys
  | _, _ => false
