...
/-- The linear part is the identity, so the rigid motion is a translation. -/
def IsTranslation (g : SE2) : Prop := g.c = 1 ∧ g.s = 0
...
/-- The action as an equivalence of the plane. -/
def toEquiv (g : SE2) : Point ≃ Point where
  toFun := g.act
  invFun := g.inv.act
  left_inv := g.inv_act_act
  right_inv := g.act_inv_act
