...
/-- Pure exact arithmetic behind the displayed area threshold `2 a₁`. -/
theorem two_a1_gt_area_threshold_of_mem_box
    {p : Params} (hp : p ∈ box) :
    ((24206448441453 : ℝ) / 10000000000000) < 2 * p.a1 := by
  have h := a1_lower_bound_of_mem_box hp
  norm_num at h ⊢
  linarith

/-- The same box bound immediately implies the coarser `2.2` threshold. -/
theorem two_a1_gt_two_point_two_of_mem_box
    {p : Params} (hp : p ∈ box) :
    (11 : ℝ) / 5 < 2 * p.a1 := by
  have h := two_a1_gt_area_threshold_of_mem_box hp
  norm_num at h ⊢
  linarith
