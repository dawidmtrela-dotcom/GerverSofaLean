...
@[simp] theorem frame_inv_zero_isTranslation (p : Params) :
    SE2.IsTranslation ((frame p 0).inv) := by
  simp [SE2.IsTranslation, frame, SE2.inv, angle]
...
/-- The standard-to-world frame acts by the manuscript formula
`q ↦ x(t) + R_t q`. -/
theorem frame_act_formula (p : Params) (s : ℝ) (q : Point) :
    (frame p s).act q =
      (Real.cos (angle s) * q.1 - Real.sin (angle s) * q.2 + (path p (angle s)).1,
       Real.sin (angle s) * q.1 + Real.cos (angle s) * q.2 + (path p (angle s)).2) := by
  rfl
